...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["application/pdf", "text/csv", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for aletheia-files" ON storage.objects FOR SELECT USING (bucket_id = 'aletheia-files');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for aletheia-files" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'aletheia-files');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for aletheia-files" ON storage.objects FOR UPDATE USING (bucket_id = 'aletheia-files');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for aletheia-files" ON storage.objects FOR DELETE USING (bucket_id = 'aletheia-files');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'aletheia-files',
        public: true,
        allowed_mime_types: ["application/pdf", "text/csv", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"],
